...
type TLS struct {
	// allocator is this TLS's private heap. When non-nil, the mem.go entry
	// points bypass the global allocMu lock and use this allocator directly.
	// Enables parallel scaling of concurrent TLSes (one per QuickJS Runtime).
...
func newTLS(detached bool) *TLS {
	...
	t := &TLS{
		ID:        id,
		errnop:    uintptr(unsafe.Pointer(&errno0)),
		allocator: &memory.Allocator{},
	}
