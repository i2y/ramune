...
// multiple goroutines.
type TLS struct {
	// allocator is this TLS's private heap. When non-nil, the mem_musl.go
	// entry points bypass the global allocatorMu lock and use this allocator
	// directly. Enables parallel scaling of concurrent TLSes (one per QuickJS
	// Runtime). Page-owner routing in Xfree/Xrealloc keeps cross-allocator
	// frees correct (see pageowner.go).
	allocator *memory.Allocator
...
func NewTLS() (r *TLS) {
	...
	// The pthread struct itself is allocated via the global allocator (nil
	// TLS) because the TLS does not exist yet. Page-owner tracking ensures
	// its eventual free in Xpthread_exit routes back to the global allocator
	// regardless of the freeing TLS's own allocator.
	...
	*(*t__pthread)(unsafe.Pointer(pthread)) = t__pthread{
	...
	return &TLS{
		...
		pthread:     pthread,
		sigHandlers: map[int32]uintptr{},
		// Fresh per-TLS allocator for parallel scaling across concurrent TLSes.
		allocator: &memory.Allocator{},
	}
}
