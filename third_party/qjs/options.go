...
type Option struct {
	...
	Stdout             io.Writer
	Stderr             io.Writer
	// DisableFS skips the WASI filesystem mount. When false (default), the
	// runtime mounts CWD at / inside the WASM module, giving QuickJS access
	// to host files via WASI. Set to true when running untrusted code — all
	// host access should flow through explicit host-import bridges rather
	// than through an ambient FS mount.
	DisableFS bool
}
