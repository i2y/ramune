...
// --- Phase 1: typeof / instanceof / in / delete ---

func TestTypeofComparison(t *testing.T) {
	path := writeTempTS(t, `
...
`)
	...
	t.Logf("output:\n%s", result.GoSource)
	// typeof x !== "number" should emit a negated type check
...
func TestTypeofStandalone(t *testing.T) {
	path := writeTempTS(t, `
...
`)
	...
	t.Logf("output:\n%s", result.GoSource)
	// Should generate a map key check
...
`)
	...
	t.Logf("output:\n%s", result.GoSource)
	// Static field should be a package-level variable
	if !strings.Contains(result.GoSource, "var Counter_Count") {
		t.Errorf("expected package-level var Counter_Count for static field")
	}
	// Instance field should still be in the struct
...
func TestStaticAccess(t *testing.T) {
	path := writeTempTS(t, `
class Config {
...
`)
	...
	t.Logf("output:\n%s", result.GoSource)
	// Getter should emit a method with no parameters
	if !strings.Contains(result.GoSource, "func (") && !strings.Contains(result.GoSource, "Name()") {
		t.Errorf("expected getter method Name() to be emitted")
	}
}
...
`)
	...
	if !strings.Contains(result.GoSource, "SetAge(") {
		t.Errorf("expected setter method SetAge() to be emitted")
	}
}
...
// --- Phase A: optional chaining, nullish coalescing, exponentiation ---
...
`)
	...
	t.Logf("output:\n%s", result.GoSource)
	// For any-typed expressions, optional chaining uses jsrt.Obj (nil-safe)
...
`)
	...
	t.Logf("output:\n%s", result.GoSource)
	if !strings.Contains(result.GoSource, "!= nil") {
		t.Errorf("expected nil check for optional call")
	}
}
...
// --- Phase B: destructuring defaults, logical assignment ---

func TestDestructuringObjectDefault(t *testing.T) {
	path := writeTempTS(t, `
interface Config {
  host: string;
  port: number;
...
func TestLogicalAndAssignment(t *testing.T) {
	path := writeTempTS(t, `
let x: any = true
x &&= "hello"
`)
...
func TestLogicalOrAssignment(t *testing.T) {
	path := writeTempTS(t, `
let x: any = null
x ||= "default"
`)
...
// --- Phase C: export default, re-exports ---
...
`)
	...
	t.Logf("output:\n%s", result.GoSource)
	if !strings.Contains(result.GoSource, "var Default") {
		t.Errorf("expected var Default for export default expression")
	}
}
...
`)
	...
	t.Logf("output:\n%s", result.GoSource)
	// export default function should export the function with its name
...
`)
	...
	t.Logf("output:\n%s", result.GoSource)
	if !strings.Contains(result.GoSource, "var Default = msg") {
		t.Errorf("expected var Default = msg")
	}
}

// --- Phase D: abstract classes, >>>, labeled statements ---
...
`)
	...
	t.Logf("output:\n%s", result.GoSource)
	if !strings.Contains(result.GoSource, "__p") {
		t.Errorf("expected __p temp var for await iteration")
	}
