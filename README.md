...
- **[Chinotto](https://github.com/i2y/chinotto)** — Self-host platform built on Ramune's env framework. A single Go binary that runs `export default { fetch, scheduled }` modules with KV / SQL / blob / durable-workflow / cron bindings. Chinotto plugs concrete backends (Pebble KV, filesystem blob, Romancy durable workflows) into Ramune's `KVBackend` / `BlobBackend` / scheduled-handler interfaces and wraps the result with a daemon (HTTP router, sandbox subprocess, TOML manifest, ops CLI).
